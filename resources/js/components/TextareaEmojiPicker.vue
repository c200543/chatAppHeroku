<template>
  ...
  <div class="textarea-emoji-picker " >
            ...
            <span @click="send()"><b-icon  icon="reply-fill" class="h1 send-btn ml-3" variant="primary" ></b-icon></span>
                    </div>
                </div>
            </div>
...
data () {
  ...
  props: {
    ...
    if (e.keyCode == 13)
    {
      ...
      $('.emoji-mart').click(function(event){
         event.stopPropagation();
});
      $('.emoji-trigger').click(function(event){
         event.stopPropagation();
});
